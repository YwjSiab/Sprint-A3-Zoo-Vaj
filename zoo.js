...
const displayAnimals = () => {
  ...
  if (!container) {
    console.error('Error: animalContainer element not found');
    return;
  }
  container.innerHTML = '';
...
const animal = animals.find(a => a.id === id);
  ...
  if (healthElement) {
    healthElement.textContent = animal.health;
  } 
  else {
    console.error(`Error: Health element for animal ID ${id} not found`);
  }
};
...
window.toggleZooStatus = () => {
  const isOpen = animals.some(animal => animal.status === 'Open');
  animals.forEach(animal => animal.status = isOpen ? 'Closed' : 'Open');
  displayAnimals();
};
